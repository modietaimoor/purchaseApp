...
export function buildUrl(baseUrl: string, path: string, pathParamsMetadata: ParameterMetadata[]): string {
  const urlTemplate = buildFullUrlTemplate(baseUrl, path);
  return replacePathParams(urlTemplate, pathParamsMetadata);
}

function buildFullUrlTemplate(baseUrl: string, path: string): string {
  if (!baseUrl) {
    return path;
  }
...
function replacePathParam(url: string, key: string, value: string): string {
  if (!value) {
